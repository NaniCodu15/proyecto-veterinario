...
function actualizarProximoNumero(lista = []) {
    ...
    if (!historiaEditandoId && numeroHistoriaInput && modalHistoria && modalHistoria.style.display === 'block') {
...
function renderHistorias(lista = null) {
    if (Array.isArray(lista)) {
        ...
        if (typeof poblarHistoriasParaCitas === 'function') {
...
async function cargarHistorias() {
    ...
    try {
        ...
        if (typeof mostrarMensajeCita === 'function') {
...
if (formConsulta) {
    formConsulta.addEventListener('submit', async event => {
        ...
        const botonGuardarConsulta = formConsulta.querySelector('[type="submit"]');
